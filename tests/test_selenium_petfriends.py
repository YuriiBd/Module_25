...
def testing():
    pytest.driver = webdriver.Chrome('/home/yurii/tests/chromedriver')
...
def test_show_my_pets():
    # Вводим email
    pytest.driver.find_element_by_id('email').send_keys('iu11@mail.ru')
    # Вводим пароль
    ...
    assert pytest.driver.find_element_by_tag_name('h2').text == "qw123"
    ...
    def test_my_pets():
        ...
        elements = WebDriverWait(pytest.driver, 10).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, '//div[@id="all_my_pets"]/table[@class="table table-hover"]/tbody/tr/td'))
        )
